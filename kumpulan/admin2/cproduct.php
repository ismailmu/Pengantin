...
//===================================================== PERINTAH INSERT UPDATE DELETE ====================================================================================================================================================================	
	...
	if($_POST['jkirim']) {
		...
		$qcontent=mysql_query("select distinct p.nama_product,p.ket_product,p.foto_1,p.foto_2,p.foto_3,p.harga from product as p where p.id_product='$_GET[ubah]$_GET[hapus]' and ( p.status_tampil='0' or ( p.status_tampil='1' and p.status_pemilik!='1' ) )");
	...
	else if( $_GET['iklan'] || $_GET['beli'] ) {
		$qmproduct=mysql_query("select nama_product,ket_product,harga from product where id_product='$_GET[iklan]$_GET[beli]'");
		$dmproduct=mysql_fetch_array($qmproduct);
		
		$qmemail=mysql_query("select email from member");
		while($dmemail=mysql_fetch_array($qmemail)) {
			@mail($dmemail[0],"Posting Product baru."," $dmproduct[0]. \n $dmproduct[1]. \n Harga : ".showRupiah($dmproduct[2]).". \n Beli sekarang dan lihat productnya di http://www.solobursasecond.com/?action=product&ncari=$dmproduct[0] ","From: admin@solobursasecond.com \r\n");
		}
		
		if( $_GET['iklan'] ) $informasi=(mysql_query("update product set status_tampil='1',tanggal='".TANGGAL."' where id_product='$_GET[iklan]'"))?"Data telah berhasil diIklankan.<br />":"Data gagal diIklankan.<br />";
		if( $_GET['beli'] ) $informasi=(mysql_query("update product set status_pemilik='1',status_tampil='1',tanggal='".TANGGAL."' where id_product='$_GET[beli]'"))?"Data telah berhasil dibeli.<br />":"Data gagal dibeli.<br />";
	}
...
//=============================================================================================================================================================================================================================================================
	...
	$qjproduct=mysql_query("select distinct p.id_product from product as p,qproduct as q where p.nama_product like '%$ncari%' and q.id_skategori like '$kcari' and p.id_product=q.id_product and ( p.status_tampil='0' or ( p.status_tampil='1' and p.status_pemilik!='1' ) )");
	...
	$qproduct=mysql_query("select distinct p.id_product,p.nama_product,p.ket_product,p.harga,p.foto_1,p.foto_2,p.foto_3,p.status_pemilik,m.nama,m.alamat,m.telepon,p.status_tampil from product as p,qproduct as q,member as m where p.nama_product like '%$ncari%' and q.id_skategori like '$kcari' and p.id_product=q.id_product and p.status_pemilik=m.email and ( p.status_tampil='0' or ( p.status_tampil='1' and p.status_pemilik!='1' ) ) order by tanggal asc limit $limit,$batas");
	...
	while($dproduct=mysql_fetch_array($qproduct)) {
		...
		$confirm=($dproduct[11]=='0')?"<a href=\"?action=$action&ncari=$ncari&kcari=$kcari&page=$_GET[page]&beli=$dproduct[0]\" target=\"_self\" class=\"bmerah\">Beli</a> || <a href=\"?action=$action&ncari=$ncari&kcari=$kcari&page=$_GET[page]&iklan=$dproduct[0]\" target=\"_self\" class=\"bmerah\">Iklan</a>":"<a href=\"?action=$action&ncari=$ncari&kcari=$kcari&page=$_GET[page]&beli=$dproduct[0]\" target=\"_self\" class=\"bmerah\">Beli</a>";
	...
	$isi.="<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"3\"><tr><td colspan=\"2\" align=\"left\">$informasi $paging</td></tr>$product<tr><td colspan=\"2\" align=\"left\">$paging</td></tr></table>";
?>