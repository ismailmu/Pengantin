<?php
	...
	$qdosen_bimbing=mysql_query("select nik_pembimbing from mahasiswa where nim='$_SESSION[user]'");
	$ddosen_bimbing=mysql_fetch_array($qdosen_bimbing);
	...
	if($_POST['kirim']) {
		...
		mysql_query("insert into konsultasi(id_konsultasi,isi,dari,untuk,tanggal,file,id_skonsultasi,status)
					values('$id_konsultasi','$isi','$_SESSION[user]','$ddosen_bimbing[nik_pembimbing]','".TANGGAL."','$file','$id_skonsultasi','1')");
	}
	...
	if($_POST['tambah'] or $_GET['detail']) {
		...
		function showKonsultasi($id_konsultasi) {
			...
			//--------------------------------------------------------------------------------------------------------
			if($ddkonsultasi['dari']!=$_SESSION['user'] ) { // menampilkan data dari dosen pembimbing tanpa menampilkan identitas dosen pembimbing
				$dari="";
				$bgcolor="";
			}
			else {
				$dari="<span class=\"bhitam\">==== Konsultasi-ku ====</span><br />";
				$bgcolor="#e5e5e5";
			}
			//--------------------------------------------------------------------------------------------------------
	...
	$qjkonsultasi=mysql_query("select distinct id_konsultasi from konsultasi where untuk='$_SESSION[user]' and dari='$ddosen_bimbing[nik_pembimbing]' $where");
	...
	$body.="			</select>
			  ...
			  <tr>
				...
				<td class=\"bputih\" align=\"center\" bgcolor=\"#4682b4\" width=\"64%\">Isi Konsultasi $_SESSION[user] $ddosen_bimbing[nik_pembimbing]</td>
	...
	$qkonsultasi=mysql_query("select distinct substr(isi,1,30) as isi,tanggal,id_konsultasi,file,status from konsultasi where untuk='$_SESSION[user]' and dari='$ddosen_bimbing[nik_pembimbing]' $where order by tanggal desc limit $limit,$batas");
	...
	$body.="  <tr>
				<td colspan=\"6\" align=\"right\" bgcolor=\"#dbeaf5\"><input type=\"hidden\" name=\"jumlah\" value=\"".($loop-1)."\" />$paging</td>
				</tr>
			</table></form><br />";
?>